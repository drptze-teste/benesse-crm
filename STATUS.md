...
- [~] **IA (Gemini) — código pronto, falta o segredo** — 2026-06-24: rotas `/api/ai/proposal-text` (escopo/e-mail) e `/api/ai/recompra-text` no server.ts (retornam 503 sem a chave). UI: botões "Escrever com IA" (escopo) e "Gerar e-mail" no ProposalModal; botão "IA" no alerta de recompra (gera msg + copiar). **PENDENTE p/ ativar:** (1) `firebase apphosting:secrets:set GEMINI_API_KEY --project crm-benesse`; (2) descomentar o bloco GEMINI_API_KEY no apphosting.yaml + push. A rota /api/ai/whatsapp-lead já existente também passa a funcionar.
