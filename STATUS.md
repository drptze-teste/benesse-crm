...
- [x] **IA (Gemini) ATIVA** — 2026-06-24: segredo `GEMINI_API_KEY` setado no Secret Manager (v2) e bloco habilitado no apphosting.yaml. Rotas `/api/ai/proposal-text` (escopo/e-mail), `/api/ai/recompra-text` e `/api/ai/whatsapp-lead` funcionando. UI: "Escrever com IA"/"Gerar e-mail" no ProposalModal; botão "IA" no alerta de recompra. Modelo `gemini-2.0-flash` (env GEMINI_MODEL).
